...
ENTITY* skycube =
{
	type = "skycube+6.png"; // the image for a backdrop scene
//	type = "clouds+6.tga";
...
action ACar_Traffic()
{
 ...
 pXent_addforcelocal( me, nullvector , _vec(5000,0,0) );
	...
	while(1)
	{
		...
		if (vec_dist(my.x,player.x)>6000){my.skill20=0; wait(60); ent_remove(me);FTrafficCount-=1;return;}
...
action ATrafficSpawner()
{
	...
	if (!you)
	{
      ...
      if (c_trace(vnode1.x,vector(vnode1.x,vnode1.y,vnode1.z-5000),IGNORE_ME | IGNORE_PASSABLE | USE_POLYGON) > 0)
		...
		if (ltypecar<1){	ent_create("350z.mdl",vnode1,ACar_Traffic);}else
		if (ltypecar<2){	ent_create("cap.mdl",vnode1,ACar_Traffic);}else
